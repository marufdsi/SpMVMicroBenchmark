...
STREAM_TYPE * __restrict__ a; STREAM_TYPE * __restrict__ b; STREAM_TYPE * __restrict__ c;
...
static char	*label[4] = {"Copy:      ", "Scale:     ",
    "Add:       ", "Triad:     "};
static char	*operations[4] = {"Copy", "Scale", "Add", "Triad"};
...
//#pragma omp parallel for
	...
	if (myrank == 0) {
		...
		AvgErrByRank = (STREAM_TYPE *) malloc(3 * sizeof(STREAM_TYPE) * numranks);
...
//#pragma omp parallel for
	...
	// Back to the original code, but now using the minimum global timing across all ranks
        ...
        std::string fileName = procs == 2 ? "CASCADE_LAKE.csv" : "SKYLAKE.csv";
        ...
        if (!infile.good()) {
            resultCSV << "Operation,Size,Bandwidth_MBS,AvgTime,MinTime,MaxTime,DataType"<< std::endl;
        }
        infile.close();
		for (j=0; j<4; j++) {
			avgtime[j] = avgtime[j]/(double)(NTIMES-1);
            resultCSV << operations[j] << "," << bytes[j] << "," << 1.0E-06 * bytes[j]/mintime[j] << "," << avgtime[j] << ","
                << mintime[j] << "," << maxtime[j] << "," << sizeof(STREAM_TYPE) << std::endl;
